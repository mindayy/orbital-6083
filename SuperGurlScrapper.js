...
const scrapeData = async () => {
    ...
    await page.goto('https://www.shopsupergurl.com/products', {
        waitUntil: 'domcontentloaded'
    });

    let results = [];
    const lastPageNumber = 21;
    ...
    for (let index = 1; index <= lastPageNumber; index++) {
        const productsOnPage = await page.evaluate(async () => {
            ...
            for (let product of elements) {
                try {
                    ...
                    const productUrl = new URL(product.querySelector('.product-title a').getAttribute('href'), 'https://www.shopsupergurl.com/').href;

                    const price = parseFloat(priceText.replace(/[^\d.]/g, '')); // converts $28.90 to just 28.9

                    // extract available sizes
                    const sizesList = Array.from(product.querySelectorAll('.product-size span.size-status-option'))
        ...
        if (index < lastPageNumber) {
            const nextPageButton = await page.$('div > ul > li.pager-next > a');
            if (nextPageButton) {
    ...
    const ref = database.ref('supergurl-products');
