...
const serviceAccount = require('/Users/min/Downloads/orbital-6083-firebase-adminsdk-qj4zv-8af0ecb393.json');
        ...
        const productsOnPage = await page.evaluate(() => {
            ...
            elements.forEach(product => {
                try {
                    ...
                    const productUrl = product.querySelector('.product-img a').getAttribute('href');

                    const price = parseFloat(priceText.replace(/[^\d.]/g, '')); // converts $28.90 to just 28.9
        
                    console.log(title, price, imageUrl, productUrl);
                    data.push({title, price, imageUrl, productUrl});
                } catch (error) {
